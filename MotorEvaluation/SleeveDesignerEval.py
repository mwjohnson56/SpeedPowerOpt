...
class SleeveProblemDef:
    def get_problem(self,state) -> 'StructuralProblem':
        design = state.design
        material_dict = {}
        for key, value in design.machine.core_mat.items():
            material_dict[key] = value
        for key, value in design.machine.magnet_mat.items():
            material_dict[key] = value
        for key, value in design.machine.sleeve_mat.items():
            material_dict[key] = value
...
class SleevePostAnalyzer:
    """Converts input state into output state for TemplateAnalyzer"""
    def get_next_state(self,results,stateIn):
        if results is False:
            raise mo.InvalidDesign(message='No sleeve found')
        if results[0]>stateIn.design.machine.d_ag:
            raise mo.InvalidDesign(message='Sleeve too thick')
        stateOut=deepcopy(stateIn)
        stateOut.design.machine.d_sl=results[0]
        return stateOut
    
    

problem_def=SleeveProblemDef()
stress_limits = {'rad_sleeve': -100E6,
                 'tan_sleeve': 1300E6,
                 'rad_magnets': 0,
                 'tan_magnets': 80E6}
analyzer=SleeveAnalyzer(stress_limits)
post_analyzer = SleevePostAnalyzer()

SleeveDesignStep=me.AnalysisStep(problem_def, analyzer, post_analyzer)